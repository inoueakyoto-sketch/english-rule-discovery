(function(){
  ...
  let state=load(),screen=state.started?"home":"intro",currentStage=null,currentProbe=null,probeHeard=false,practiceSession=null,practiceItem=null,selectedSound=null,memorySession=null,memoryItem=null,memoryHeard=false,toastTimer=null;
  ...
  function renderHome(){
    if(completed()){els.discoverTitle.textContent="読みの発見はひと区切り";els.discoverText.textContent="基本パターンと『例外もある』まで発見済み。音の地図でいつでも復習できます。";}else{const s=stage();els.discoverTitle.textContent="読み方を発見する";els.discoverText.textContent=`次は ${s.code}。まだ答えは見せず、単語を聞いて比べます。`;}
  ...
  function completeDiscovery(){if(!probeHeard)return;state.stageIndex=Math.min(B.stages.length,state.stageIndex+1);save();toast("音の地図に保存しました");if(completed())showMap();else showHome();}

  function showMap(){screen="map";hideCards();show(els.map);els.mapList.innerHTML="";const found=B.stages.slice(0,Math.min(state.stageIndex,B.stages.length));if(!found.length)els.mapList.innerHTML='<p class="phonics-empty-mastery">まだ発見はありません。</p>';
    found.forEach(s=>{const item=document.createElement("article");item.className="phonics-map-item";item.innerHTML=`<div class="phonics-map-item-head"><span>${esc(s.code)}</span><b>${esc(s.short)}</b>${s.sound?`<strong>${esc(s.sound)}</strong>`:""}</div><p>${esc(s.discover)}</p><div class="phonics-map-examples">${s.examples.slice(0,4).map(w=>`<button type="button" data-word="${esc(w.word)}">${esc(w.word)} <small>▶</small></button>`).join("")}</div><small class="phonics-map-note">${esc(s.note)}</small>`;item.querySelectorAll("button").forEach(b=>b.addEventListener("click",()=>speak(b.dataset.word)));els.mapList.appendChild(item);});refresh();scrollToTop();
  }

  function availablePracticeStages(){return B.stages.slice(1,Math.min(state.stageIndex,B.stages.length)).map(s=>s.id);}
  function startPractice(){const ids=availablePracticeStages();if(!ids.length){toast("まず読み方を1つ発見すると練習できます");return;}practiceSession={index:0,correct:0,results:[],recent:[]};nextPractice();}
  function nextPractice(){if(practiceSession.index>=PRACTICE_LENGTH)return showPracticeComplete();const ids=availablePracticeStages(),stageId=C.chooseStage(ids,state,practiceSession.recent),pool=B.practiceWords.filter(w=>w.stageId===stageId);practiceItem=pool[Math.floor(Math.random()*pool.length)];practiceSession.recent.push(stageId);selectedSound=null;screen="practice";hideCards();show(els.practice);renderPractice();refresh();scrollToTop();}
  function renderPractice(){const s=B.stages.find(x=>x.id===practiceItem.stageId);els.practiceCounter.textContent=`${practiceSession.index+1} / ${PRACTICE_LENGTH}`;els.practiceDots.innerHTML="";for(let i=0;i<PRACTICE_LENGTH;i++){const d=document.createElement("span");if(i<practiceSession.results.length)d.className=practiceSession.results[i].correct?"done good":"done miss";else if(i===practiceSession.index)d.className="now";d.textContent=i+1;els.practiceDots.appendChild(d);}els.practiceFocus.textContent="PATTERNを見て予想";els.practiceWord.innerHTML=highlight(practiceItem.word,focusFor(s,practiceItem));els.practiceMeaning.textContent=practiceItem.meaning;hide(els.practiceMeaning);els.meaningBtn.textContent="意味を見る";els.soundChoices.innerHTML="";const correctFamily=B.familyForWord(practiceItem),correctChoice=B.soundChoices.find(c=>c.id===correctFamily),distractors=B.soundChoices.filter(c=>c.id!==correctFamily).sort(()=>Math.random()-.5).slice(0,3),visible=[correctChoice,...distractors].filter(Boolean).sort(()=>Math.random()-.5);visible.forEach(c=>{const b=document.createElement("button");b.type="button";b.className="phonics-sound-choice";b.innerHTML=`<b>${esc(c.label)}</b><small>${esc(c.sub)}</small>`;b.addEventListener("click",()=>{selectedSound=c.id;els.soundChoices.querySelectorAll("button").forEach(x=>x.classList.toggle("selected",x===b));els.practiceCheck.disabled=false;});els.soundChoices.appendChild(b);});els.practiceCheck.disabled=true;}
