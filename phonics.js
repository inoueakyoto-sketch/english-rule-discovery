(function(){
  ...
  function startDiscovery(){if(completed())return showMap();currentStage=stage();screen="discover";hideCards();show(els.discover);els.stageCode.textContent=currentStage.code;els.stageTitle.textContent=currentStage.title;els.stageLead.textContent=currentStage.lead;els.stageCounter.textContent=`${currentStage.order+1} / ${B.stages.length}`;els.examples.innerHTML="";
    currentStage.examples.forEach(w=>{const b=document.createElement("button");b.type="button";b.className="phonics-word-card";b.innerHTML=`<span class="phonics-word-text">${highlight(w.word,w.focus)}</span><small>${esc(w.meaning)}</small>${w.soundCue?`<em>${esc(w.soundCue)}</em>`:""}<i><span aria-hidden="true">◖)))</span> 聞く</i>`;b.setAttribute("aria-label",`${w.word}、${w.meaning}。音を聞く`);b.addEventListener("click",()=>speak(w.word));els.examples.appendChild(b);});
