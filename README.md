# DISCOVERY v0.2.2 sound-discovery-table candidate

GitHub Pagesでそのまま公開できる、スマホ中心の中学生向け英語学習Webアプリです。v0.2.2では「静かな発見」の世界観を保ったまま、WORD CODEに「音の発見表」を追加しました。発見した規則だけが表に現れ、例外は後から追記され、各行からその音だけを5問練習できます。


## v0.2.2 — 静かな発見 + 音の発見表

### 音の発見表
- 未発見の規則は `？` のまま。先の答えを見せません。
- 発見済みは「つづり / 音 / 例 / 発見番号」で表に蓄積します。
- 例外ステージを発見すると `ea` / `a_e` / `i_e` に別読みのメモが後から追記されます。
- 発見済みの行から「音を聞く」「この音を5問」が使えます。
- 下部の DISCOVERY LOG では、発見した順に詳しい説明へ戻れます。
...
```bash
git add .
git commit -m "Update to v0.2.2 sound discovery table"
git push
```
